...
bool safepoints(int** arr,int x,int y, int n){
    if(x<n && y<n && arr[x][y]==1){     // defferentiate  between the obstracles and given free of obstracle path.
        return true;
    }
    return false;
}
bool RatinMaze(int** arr,int x,int y,int n,int** SolutionArr){
    ...
    if (safepoints(arr,x,y,n)){
        SolutionArr[x][y]=1;   // indicates that this can be a path   if it will not we will backtrack and assign this the value =0;
        ...
        SolutionArr[x][y]=0;      //backtrackpoint    indicated path future me ja k fail ho gya.
        return false;
    }
